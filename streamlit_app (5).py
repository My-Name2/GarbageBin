    ...
    with tabs[1]:
        ...
        for chart_idx in range(num_charts):
            ...
            with col:
                ...
                filter_pos = st.checkbox(
                    "Filter > 0", value=False, key=f"chart_{chart_idx}_filter_pos"
                )
                filter_thresh = st.number_input(
                    "Filter <", value=100.0, step=1.0, key=f"chart_{chart_idx}_filter_thresh"
                )
                ...
                if series_raw.empty:
                    ...
                    continue
                # Apply filtering based on user choices
                series = series_raw.copy()
                if filter_pos:
                    series = series[series > 0]
                if filter_thresh is not None and filter_thresh != 0:
                    series = series[series < filter_thresh]
                if series.empty:
